...
public class SoomlaSettingsEditor : Editor
{
	...
	GUIContent customSecLabel = new GUIContent("Custom Secret [?]:", "The application encryption secret. (same as in the export screen on dashboard)");
	GUIContent soomSecLabel = new GUIContent("SoomSec [?]:", "A global secret which is used as a higher level protection. (same as in the export screen on dashboard)");
	GUIContent masterKeyLabel = new GUIContent("Master Key [?]:", "The server side key associated with your account. (key icon on storefront in the dashboard)");

	GUIContent publicKeyLabel = new GUIContent("Play API Key [?]:", "The API key from Google Play dev console. (same as in the export screen on SOOMLA dashboard)");
    ...
    private void SoomlaGUI()
    {
		...
		EditorGUILayout.BeginHorizontal();
		EditorGUILayout.LabelField(masterKeyLabel, fieldWidth, fieldHeight);
		SoomSettings.MasterKey = EditorGUILayout.TextField(SoomSettings.MasterKey, fieldHeight);
		EditorGUILayout.EndHorizontal();
        ...
        if (showAndroidSettings)
        {
			...
			if (!SoomlaAndroidUtil.IsSetupProperly())
			{
				...
				case SoomlaAndroidUtil.ERROR_NO_KEYSTORE:
					...
					break;
				case SoomlaAndroidUtil.ERROR_NO_JAVA:
					msg = "Can't run the 'java' command. Add your java JDK folder to the PATH environment variable.";
					break;
				case SoomlaAndroidUtil.ERROR_NO_JARSIGNER:
					msg = "Can't run the 'java' command. Add your java JDK's bin folder to the PATH environment variable.";
					break;
				}
